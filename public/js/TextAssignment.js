function triggerTextFileInput(){
    document.getElementById('textfile-upload').click();
}
...
function handleText(event) {
    ...
    if (file) {
        ...
        fetch('http://127.0.0.1:8000/get-text-uploaded-files', {
            method: 'GET',
            headers: {
        ...
        .then(data => {
            if (data.success) {
                ...
                fetch('http://127.0.0.1:8000/upload-text-assignment', {
                    method: 'POST',
                    body: formData,
                    headers: {
                        'Accept': 'application/json',
...
function fetchTextUploadedFiles() {
    fetch('http://127.0.0.1:8000/get-text-uploaded-files', {
        method: 'GET',
        headers: {
            'Accept': 'application/json',
            ...
            data.files.forEach(file => {
                if (file.id) {
                    ...
                    listItem.innerHTML = `File: <strong>${file.file_name}</strong> | Uploaded At: ${file.created_at}
                        <button onclick="deleteTextFile(${file.id})" style="margin-left: 10px; color: white; background-color: red; border: none; padding: 5px 10px; border-radius: 4px; cursor: pointer;">
                            Delete
                        </button>
                    `;
...
window.onclick = function(event) {
    ...
    if (event.target === modal) { // Detect clicks on the background (outside modalContent)
        closeModal();
    }
};
...
function deleteTextFile(fileId) {
    ...
    fetch(`http://127.0.0.1:8000/delete-text-file/${fileId}`, {
        method: 'DELETE',
        headers: {
            'Accept': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
    ...
    .then(data => {
        if (data.success) {
            alert(data.message);
            fetchTextUploadedFiles(); // Refresh the file list after deletion
        } else {
...
function checkTextPlagiarism() {
    ...
    .then(data => {
        if (data.success && data.files.length === 2) {
            ...
            fetch('http://127.0.0.1:8000/check-text-plagiarism', {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
...
// Function to display plagiarism report in the DOM
        ...
        ">
            ...
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
                <tr>
                    ...
                    <td style="text-align: right; padding: 10px;">${fileNames[0]} (${fileTypes[0]})</td>
                </tr>
                <tr>
                    <th style="text-align: left; padding: 10px;">Assignment 2</th>
                    <td style="text-align: right; padding: 10px;">${fileNames[1]} (${fileTypes[1]})</td>
                </tr>
                <tr>
